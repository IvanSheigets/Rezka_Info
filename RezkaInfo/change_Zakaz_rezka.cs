...
namespace RezkaInfo
{
    public partial class change_Zakaz_rezka : Form
    {
        ...
        public change_Zakaz_rezka(int iZakazID)
        {
            if (iZakazID > 0)
                m_iZakazID = iZakazID;
        ...
        private void SelectZakaz(int iZakazID)
        {
            if (CheckConnect())
            {
                strMSSQLQuery = "select partiya, productmaterial_id, producttols_id, dlinaetiketki, smena, machine from itak_etiketka.dbo.itak_zakaz where id=" + iZakazID;
                try
                {
                    m_MSSQLCommand.CommandText = strMSSQLQuery;
                    m_MSSQLReader = m_MSSQLCommand.ExecuteReader();
