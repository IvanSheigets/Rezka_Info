...
namespace RezkaInfo
{
    public partial class rylon_form : Form
    {
        ...
        repairRylon_form m_repairRylonForm = null;
        ...
        private void rylon_form_Load(object sender, EventArgs e)
        {
            ...
            if (m_iTypeRegim == 2 || m_iTypeRegim==3)
            {
                button_all_label.Visible = true;
                checkbox_logo.Visible = true;
            }

            if (m_iTypeRegim == 0 || m_iTypeRegim==2 || m_iTypeRegim==3)
        ...
        {
            if (/*m_iTypeRegim == 1 ||*/ m_iTypeRegim == 2 || m_iTypeRegim==3 )
            {
                int iRowSelect = -1;
                for (int i = 0; i < dataGridView_rylon.Rows.Count; i++)
                ...
                if (iRowSelect != -1)
                {
                        ...
                        else if (m_delChange.GetDialogResult() == 2)
                        {
                            if (MessageBox.Show(this, "Вы действительно хотите удалить рулон №" + dataGridView_rylon.Rows[iRowSelect].Cells["Num_rylon"].Value.ToString() + "??", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                            {
                                if (CheckConnect())
                                {
                                    ...
                                    else if (m_iTypeRegim == 2 || m_iTypeRegim==3)
                    ...
                    else if (m_iTypeRegim==3)
                    {
                        m_repairRylonForm = new repairRylon_form();
                        m_repairRylonForm.ShowDialog();

                        if (m_repairRylonForm.GetDialogResult() == 1)
                        {
                            string strKey = dataGridView_rylon.Rows[iRowSelect].Cells["Num_rylon"].Value.ToString() + " - " +
                                                   dataGridView_rylon.Rows[iRowSelect].Cells["Brytto"].Value.ToString() + " - " +
                                                   dataGridView_rylon.Rows[iRowSelect].Cells["Netto"].Value.ToString() + " - " +
                            ...
                            if (CheckConnect())
                            {
                                DateTime dt = System.DateTime.Now;
                                string strCheckTime = dt.Year + "." + dt.Day + "." + dt.Month + " " + dt.Hour + ":" + dt.Minute + ":" + dt.Second;
                                strMSSQLQuery = "update itak_etiketka.dbo.itak_vihidrylon set state_rylon=0, check_men=0, check_time=CONVERT (SMALLDATETIME, '" + strCheckTime + "',103) where id=" + iRylonID;
